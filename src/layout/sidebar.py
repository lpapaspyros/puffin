...
def get_refactor_options():
    ...
    with st.expander(":twisted_rightwards_arrows: **Refactor Options**", expanded = True):
        ...
        if refactor_options["programming_language"] == "SQL":
            refactor_options["sql_variant"] = generate_dropdown(
                label = "SQL Variant",
                options = refactor_menu_options["sql_variants"]
            )   
            
        refactor_options["optimize_for"] = generate_multiselect(
            label = "Optimize for",
            options = refactor_menu_options["optimize_for"]
        )
